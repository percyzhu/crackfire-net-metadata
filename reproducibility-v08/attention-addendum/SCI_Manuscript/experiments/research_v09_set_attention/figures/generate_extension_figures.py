...
def draw_figures(plan,means,seeds,effects,counts,output):
    ...
    fig.text(.60,.095,'MAE (×10⁻³ response-index units)',ha='center',fontsize=9)
    ...
    fig,axes=plt.subplots(1,2,figsize=(180/25.4,136/25.4),sharex=True,sharey=True)
    fig.subplots_adjust(left=.235,right=.985,bottom=.17,top=.88,wspace=.20)
    for j,(ax,tag,baseline) in enumerate(zip(axes,CONTRASTS,BASELINES)):
        ...
        ax.set_title(CONTRAST_LABELS[j],pad=9);ax.set_xlim(-limit,limit);ax.set_ylim(11.55,-.55)
        ax.set_yticks(y);ax.set_yticklabels(names);ax.tick_params(axis='y',length=0,labelleft=j==0)
        ax.tick_params(axis='x',labelsize=7.5);ax.grid(axis='x',color='#E3E8EB',lw=.5);ax.set_axisbelow(True)
        ax.axhline(1.5,color='#ADBCC5',lw=.8);ax.text(-.06,1.10,chr(97+j),transform=ax.transAxes,weight='bold',fontsize=10)
    fig.text(.60,.105,'Paired MAE difference (×10⁻³); 95% crossed-bootstrap interval',ha='center',fontsize=8.5)
    fig.text(.60,.055,'Negative: comparator favored     |     Positive: attention favored',ha='center',fontsize=7.7)
    ...
    for name,figure in figures:
        ...
        qa.append({'figure':name,'canvas_width_mm':180,'canvas_height_mm':136,'text_outside_canvas':outside,
...
def main(argv=None):
    ...
    try:
        ...
        sources=[Path(__file__),HERE/'FIGURE_CONTRACT.md',HERE/'captions_en.tex',HERE/'methods_extension_draft.tex']
