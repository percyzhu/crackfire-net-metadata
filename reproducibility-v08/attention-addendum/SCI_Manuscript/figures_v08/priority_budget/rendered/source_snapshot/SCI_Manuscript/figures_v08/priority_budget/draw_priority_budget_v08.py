...
def main():
    ...
    for model in MODELS:
        ...
        for i,p in enumerate(PROTOCOLS):
            for j,(endpoint,budget) in enumerate(cols):
                ...
                if defined: label=label.removesuffix('.0')
    ...
    write(out/'provenance.json',{'status':'ACTUAL_FULL_MATRIX_RENDERED_PENDING_INDEPENDENT_FIGURE_AND_VISUAL_QA',
          ...
          'meaningful_negative_cells':int((finite < -1e-8).sum()),'display_rounding':'at most one decimal; redundant .0 omitted and rounded negative zero printed 0; raw retained',
