"""Audit final-size text/vector export and create a PDF-derived QA preview."""
...
pdf = HERE / 'transfer_results_v08.pdf'
doc = fitz.open(pdf)
assert len(doc) == 1
page = doc[0]
...
assert abs(size_mm[0] - 183) < .001 and abs(size_mm[1] - 174) < .001
assert font_min >= 7.99999
assert len(page.get_images(full=True)) == 0
for text in ['source 005 only', 'no refitting', 'Radius', 'kNN', 'same trained weights']:
